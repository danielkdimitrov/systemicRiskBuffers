...
class PDmodel:
    def __init__(self, varyParam, rho_base = .9, paramsDict = False):       
        '''
        ...
        'base case'
        if varyParam == 'EEI opt w/data' or varyParam == 'min ES' or varyParam == 'Opt k_bar':
            ...
            'Reference bank : '
            self.sigma_ref = np.min( rho_base['Sigma'] )
            self.EAD_ref = .05 #np.min( rho_base['wts'] )
            self.LGD_ref = .2 #np.min(rho_base['LGD'])
        ...
        '--------- Vary Rho'
        if varyParam == 'rho':
            #self.sysIndex = np.ones(10) ==1  # index of which institutions are systemic                       
        ...
        if varyParam == 'k':
            ...
            #self.sysIndex = np.concatenate((np.array([1, 0]), np.zeros(8)), axis=0) ==1  # index of which institutions are systemic 
            self.EAD = np.concatenate((np.array([.5, .2]), np.ones(8)*(.3/8)), axis=0)
            self.sysIndex = np.concatenate((np.array([1, 1]), np.zeros(8)), axis=0) ==1  # index of which institutions are systemic
        ...
        if varyParam == 'EEI':
            ...
            'Find the solution'
            self.dict['k_i_str'] = self.getEEIstar(self.k_base,self.U_base, self.EAD) 
        if varyParam == 'EEI Multivariate':
            self.EAD = np.concatenate((np.array([.5, .2]), np.ones(8)*(.3/8)), axis=0)
            self.sysIndex = np.concatenate((np.array([1, 1]), np.zeros(8)), axis=0) ==1  # index of which institutions are systemic
            
            #self.dict['k_str'] = self.getEEImulti(self.k_base,self.U_base, self.EAD) 
        ...
        if varyParam == 'EEI opt w/data':
            ...
            for j in range(self.nOpt):
                x0[j] = self.getEEIstar(self.k_base,self.U_base, self.EAD, j)
        ...
        if varyParam == 'min ES':
            ...
            self.k_bar = np.sum(self.EAD*self.current_k)/self.nOpt 
            self.dict = {}
            
            'sum squared residuals'
            x0 = np.ones(self.nOpt)*self.k_bar
            ...
            self.dict["ESmicro"], self.dict['MESmicro'] = self.getES(np.zeros(4) ,self.U_base,True)
    ...
    def getVaryRho(self,rho_grid):
        ...
        PDdict["MESi"], PDdict["ES"], PDdict["PDi_sys"] = np.zeros_like(rho_grid), np.zeros_like(rho_grid), np.zeros_like(rho_grid)        
        PDdict["k_i_str"] = np.zeros_like(rho_grid)
        
        for jRho, rho in enumerate(rho_grid):
            U = self.factorSim(rho)
            Xrho, PDrho = self.DD(self.k_base, self.Sigma_base, self.r)
            IndD, IndS = self.defaultSims(U,Xrho)
            PD, PDj_i, ENd, ENd_i, PDi_sys = self.defaultCrossCorrs(IndD, IndS)
            IndirectCost, DirectCost, SCDi = self.GetSCD(PD, PDj_i, self.LGD, self.EAD, 0)
            nSystemic = len(self.sysIndex[self.sysIndex==True])
            ES, MES = self.getES([0.]*nSystemic ,U, True)
            ...
            PDdict["MESi"][jRho], PDdict["ES"][jRho], PDdict["PDi_sys"][jRho] = MES[0], ES, PDi_sys[0]           
            
            PDdict["rho"][jRho] = rho
            #PDdict["k_i_str"][jRho] = self.getEEIstar(self.k_base,U, self.EAD) 
    ...
    def getVarryK(self,k_macro_grid):
        ...
        PDdict["MESi"], PDdict["ES"], PDdict["PDi_sys"] = np.zeros_like(k_macro_grid), np.zeros_like(k_macro_grid), np.zeros_like(k_macro_grid)        

        k_i = self.k_base.copy() 
        ...
        for jK, k_macro in enumerate(k_macro_grid):
            ...
            PD, PDj_i, ENd, ENd_i, PDi_sys = self.defaultCrossCorrs(IndD, IndS)
            IndirectCost, DirectCost, SCDi = self.GetSCD(PD, PDj_i, self.LGD, self.EAD, 0)
            ES, MES = self.getES(k_macro , self.U_base, True)

            
            PDdict["ENd"][jK], PDdict["ENd_i"][jK] = ENd, ENd_i
            PDdict["MESi"][jK], PDdict["ES"][jK], PDdict["PDi_sys"][jK] = MES[0], ES, PDi_sys[0]           
            ...
            #PDdict["k_i_str"][jK] = self.getEEIstar(self.k_base + k_macro, self.U_base, self.EAD) 
    ...
    def getVaryW(self,w_grid):
        ...
        PDdict["MESi"], PDdict["ES"], PDdict["PDi_sys"] = np.zeros_like(w_grid), np.zeros_like(w_grid), np.zeros_like(w_grid)        
        ...
        for jRho, w_i in enumerate(w_grid):
            ...
            IndirectCost, DirectCost, SCDi = self.GetSCD(PD, PDj_i, self.LGD, EAD_now, 0)
            ES, MES = self.getES( 0., self.U_base, True)
            ...
            'evaluate k^*'
            #PDdict["k_i_str"][jRho] = self.getEEIstar(self.k_base, self.U_base, EAD_now)             
    ...
    def getVaryEEI(self, k_macro_grid):
        ...
        PD, PDj_i, ENd, ENd_i, ELi_sys = self.defaultCrossCorrs(IndD, IndS)
        IndirectCost, DirectCost, SCDi = self.GetSCD(PD, PDj_i, self.LGD, self.EAD, ELi_sys, 0)        
        ...
        for jK, k_macro in enumerate(k_macro_grid):
            ...
            PD, PDj_i, ENd, ENd_i, ELi_sys = self.defaultCrossCorrs(IndD_macro, IndS_macro)
            IndirectCost, DirectCost, SCDi = self.GetSCD(PD, PDj_i, self.LGD, self.EAD, ELi_sys, 0)
    ...
    def getVaryEEImulti(self, k1_macro_grid, k2_macro_grid):
        ...
        PDdict["MES1"], PDdict["ES1"], PDdict["PD1_sys"] = np.zeros([self.nGrdPnts,self.nGrdPnts]), np.zeros([self.nGrdPnts,self.nGrdPnts]), np.zeros([self.nGrdPnts,self.nGrdPnts])        
        ...
        PDdict["MES2"], PDdict["ES2"], PDdict["PD2_sys"] = np.zeros([self.nGrdPnts,self.nGrdPnts]), np.zeros([self.nGrdPnts,self.nGrdPnts]), np.zeros([self.nGrdPnts,self.nGrdPnts])   
        ...
        for jK1, k1_macro in enumerate(k1_macro_grid):
            ...
            for jK2, k2_macro in enumerate(k2_macro_grid):
                ...
                IndD_macro, IndS_macro = self.defaultSims(self.U_base,Xk_macro)
                ES, MES = self.getES( [k1_macro, k2_macro], self.U_base, True)
                
                
                'bank 1'
                PD, PDj_i, ENd, ENd_i, ELi_sys = self.defaultCrossCorrs(IndD_macro, IndS_macro, i = 0)
                ...
                PDdict["MES1"][jK1,jK2], PDdict["ES1"][jK1,jK2]= MES[0], ES        
                
                #PDdict["ENd"][jK1,jK2], PDdict["ENd_i"][jK1,jK2] = ENd, ENd_i
                
                'bank 2'
                PD, PDj_i, ENd, ENd_i, ELi_sys = self.defaultCrossCorrs(IndD_macro, IndS_macro, i = 1)
                ...
                PDdict["MES2"][jK1,jK2], PDdict["ES2"][jK1,jK2] = MES[1], ES        
    ...
    def getResidMulti(self,k_macro, getSCDs=False):
        ...
        for i in range(self.nOpt):
            ...
            PD, PDj_i, ENd, ENd_i, ELi_sys = self.defaultCrossCorrs(IndD, IndS, jSys)        
            IndirectCost[i], DirectCost[i], SCDi[i] = self.GetSCD(PD, PDj_i, self.LGD, self.EAD, ELi_sys, jSys) 
    ...
    def getEEIstar(self, k_micro , U ,  EAD, i=0 ):        
        # 'Get BM Impact'
        # EAD_ref= .1 #EAD[0] #benchmark relative weight
        # LGD_ref = 1.
        # 'using the first institution as a reference'
        # X_refM, self.PD_ref = self.DD(k_micro[0], self.sigma_ref, self.r)
        
        # self.SCD_ref = self.PD_ref*LGD_ref*EAD_ref
    ...
    def Resid(self,k_i_macro, k_micro, U, EAD, i=0):
        ...
        PD, PDj_i, ENd, ENd_i, ELi_sys = self.defaultCrossCorrs(IndD, IndS, i)
        
        IndirectCost, DirectCost, SCDi = self.GetSCD(PD, PDj_i, self.LGD, EAD, ELi_sys, i) 
    ...
    def defaultCrossCorrs(self, IndD, IndS, i = 0):
        ''''
        ...
        PDi_sys = np.average(IndD.T[Lsys>self.Lbar],axis=0)
        Psys = Lsys[Lsys>self.Lbar]
        ...
        '''  
        #sumEj_si = np.sum(Ej_si)/self.nBanks ELi_sys
        return PD, PDj_i, ENd, ENd_i, PDi_sys
