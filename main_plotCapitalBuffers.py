...
#%% LOCAL SCALE
'load values'
dfTable = pd.read_csv(r'output\OSIIrateOpt.csv', index_col=0)
#dfTable.drop('EEI', axis=1, inplace = True)

#dfTable = pd.read_csv('OsiiESS.csv', index_col='Short Code')
dfTable = dfTable[['Country','k_osii','k_macro_str_osii_local']]
...
plt.legend(['O-SII Rate','ESS Approach (Local)'], fontsize = 14,loc='lower center', ncol=3,bbox_to_anchor=(0.5, -0.32))
...
plt.ylim([0,3])  # Adjust font size here
...
#dfTable = pd.read_csv('OsiiESS.csv', index_col='Short Code')
dfTable = dfTable[['Country','k_macro_str_osii_eur','k_macro_str_osii_local']]
...
plt.gca().xaxis.set_ticks_position('none')
plt.gca().tick_params(axis='x', which='minor', bottom='off')
plt.legend(['ESS Approach (Euro)','ESS Approach (Local)'], fontsize = 14,loc='lower center', ncol=3,bbox_to_anchor=(0.5, -0.32))
#plt.legend(['ESS (Euro-wide)','ESS (Country-wide)'], fontsize = 14,loc='upper center', ncol=3,bbox_to_anchor=(0.5, -0.25))
